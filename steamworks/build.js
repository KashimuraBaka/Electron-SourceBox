...
const target =
  targets[process.argv.at(-1)] ||
  Object.values(targets).find((t) => t.platform === process.platform && t.arch === process.arch)

const dist = path.join(__dirname, '../resources', target.folder)
...
const params = [
  ...
  'false',
  '--dts',
  '../../src/main/utils/steamworks/client.d.ts',
  relative,
  process.argv.slice(2).join(' ')
]

child_process.spawn('napi', params, { stdio: 'inherit', shell: true }).on('exit', (err) => {
  if (err) {
    throw err
  }
})
